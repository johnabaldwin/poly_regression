...
def test_format(
    ...
    results_file = tmp_path / f"results_{fmt.name}.json"
    build_dir    = tmp_path / f"sim_build_{fmt.name}"
    ...
    # ── Accumulate for CSV ────────────────────────────────────────────────
    row: dict = {"format": fmt.name, "cycles": hw["cycles"]}
    ...
    w = fmt.width
    print(f"\n{'='*64}")
    print(f"  {fmt.name} ({w}-bit)  |  {hw['cycles']:,} cycles")
